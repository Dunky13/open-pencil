...
const FRAMEWORKS: Record<
  StorybookFramework,
  { storybook: string; imports: es.Node[]; render: es.Node }
> = {
  react: {
...
function argsObject(data: StoryModuleData, values: string[]): es.Node {
  return es.object(data.props.map((prop, i) => [prop.name, es.string(values[i] ?? '')]))
}

function parameters(design: StoryDesign[]): es.Node {
...
function argsType(data: StoryModuleData): es.Node {
...
/** `new URL('<path>', import.meta.url)` design image paths a story module references. */
export function storyImagePaths(source: string): string[] {
  let program: es.Node
  try {
    program = es.parseModule(source)
  } catch {
    return []
  }
  return [...es.walk(program)].flatMap((node) => {
    if (node.type !== 'NewExpression' || es.child(node, 'callee')?.name !== 'URL') return []
    const args = es.children(node, 'arguments')
    const [path, base] = [args.at(0), args.at(1)]
    const isMetaURL =
      es.child(base, 'object')?.type === 'MetaProperty' &&
      es.child(base, 'property')?.name === 'url'
    return isMetaURL && path?.type === 'Literal' && typeof path.value === 'string'
      ? [path.value]
      : []
  })
}
